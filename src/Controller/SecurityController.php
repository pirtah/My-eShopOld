...
class SecurityController extends AbstractController
{
    ...
    public function register(Request $request, EntityManagerInterface $em, UserPasswordHasherInterface $passwordHash): Response 
    {
        ...
        if($form->isSubmitted() && $form->isValid()) {
            // méthode présente dans le use TimestampableEntity
            $user->setCreatedAt( new \DateTime);
            $user->setUpdatedAt( new \DateTime);
            $user->setRoles(['ROLE_USER']);
            // pour récupérer le champ du formulaire
            //dd($form->get('password')->getData());
            // il faut set manuellement le hash du password grâce à la méthode hashPassword
